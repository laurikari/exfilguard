...
- `full/` mirrors a multi-tenant deployment with several CIDR-mapped clients and
  ...
  full feature set.
- `quickstart/` allows `127.0.0.1` to reach `https://www.searchkit.com/**`
  (pass-through CONNECT), plus a deny-all fallback. It’s ideal for smoke tests
  because curl doesn’t need to trust the generated CA.
